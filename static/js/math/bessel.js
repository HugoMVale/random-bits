...
export const bessel = {
    ...
    J(n, x) {
        ...
        if (x < 0) return (n % 2 === 0 ? 1 : -1) * this.J(n, -x);

        if (n === 0) return this._integralJ(0, x);
        if (n === 1) return this._integralJ(1, x);
    ...
    getZero(n, m) {
        ...
        if (!this.zerosCache[n]) this.zerosCache[n] = [];
        ...
        let rootsFound = 0;
        while (cache[rootsFound + 1] !== undefined) rootsFound++;
